...
class NavigationEnv(IsaacEnv):
    ...
    def __init__(self, cfg):
        ...
        # LiDAR Intialization
        # dynamic_mesh_prim_paths = [f"/World/Origin{i}" for i in range(cfg.env_dyn.num_obstacles//2)] + [f"/World/Origin{i}" for i in range(cfg.env_dyn.num_obstacles//2, cfg.env_dyn.num_obstacles)]
        ...
        with torch.device(self.device):
            ...
            self.dt = 0.01  # 仿真时间步长
            self.target_vel = torch.zeros(self.num_envs, 1, 3)
            self.max_speed = 0.5  # 目标最大移动速度
            self.boundary = 5.0   # 水平面移动边界
    ...
    def update_target_pos(self):
        ...
        # print("target_pos: ", self.target_pos[0][0])
    ...
    def reset_target(self, env_ids: torch.Tensor):
        if (self.training):
            ...
            target_pos = target_pos * selected_masks
            
            # apply target pos
            self.target_pos[env_ids] = target_pos
            # self.debug_draw.clear()
            # self.debug_draw.plot(self.target_pos.squeeze(1))
            ...
            # direction /= torch.norm(direction, dim=1, keepdim=True)  # 归一化方向
            
            # # 设置恒定速度（0.3-0.5 m/s）
            # speed_scale = torch.rand(self.num_envs, 1, device=self.device) * 0.2 + 0.3
    ...
    def _compute_state_and_obs(self):
        ...
        # -----------------Network Input Final--------------
        # set dyn_obs_states to zero
        # dyn_obs_states = torch.zeros_like(dyn_obs_states)
        ...
        distance_threshold = 1.0  # 距离阈值（当距离小于该值时触发减速惩罚）
        decel_weight = 1.0         # 减速惩罚的权重（根据训练效果调整）

        # 计算当前速度的模长（标量速度）
        current_speed = self.drone.vel_w[..., :3].norm(dim=-1, keepdim=True)  # shape: [num_envs, 1]

        # 计算减速惩罚项
        penalty_decel = torch.where(
            distance < distance_threshold,          # 条件：距离是否在阈值内
            (current_speed ** 2) / (distance + 1e-6),  # 惩罚项：速度平方 / 距离（避免除零）
            torch.zeros_like(current_speed)           # 超出阈值时惩罚为0
        ) * decel_weight  # 应用权重
        ...
        reach_goal = (distance.squeeze(-1) < 0.1)
        ...
        # print("return: ", self.stats["return"])
